...
Code `09c2240` is pushed in draft [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19). [All 9 CI jobs passed](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36259475980), including an actual beta0.9.1 → beta0.9.3 installation upgrade, saved-data preservation and backup restoration. The PR is not merged, and public downloads remain beta0.9.2.

**These changes are not publicly released. The download above is still beta0.9.2; local beta0.9.3 installer and portable candidates have been built.** The home, farm, companion room and study screenshots below come from the same candidate with a separate test save. Key browser workflows and build contents have been checked. The CI build also passed actual Windows installation, upgrade and backup restoration checks. A real system notification appearing and launching after Windows login with startup enabled still need validation.
