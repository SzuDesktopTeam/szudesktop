...
function reactPet(action,notifyDesktop=true){const pet=activePet(state.game),reaction=petReaction(action,pet);if(reaction)petPlayer?.play(reaction);if(reaction&&notifyDesktop)globalThis.szuDesktop?.petResult?.({ok:true,message:pet.say,action:reaction})}
async function puzzleAction(type,extra={}){return run(async()=>{
 ...
 const next=act(state,{type,...extra});
 ...
 if(type==='puzzleClaim'){reactPet(type);toast('收到今日小礼物：8 荔枝币、2 点亲密度')}
},false)}
...
function showReward(reward){const el=$('#toast');clearTimeout(toastTimer);el.classList.add('reward-toast','show');el.innerHTML=sprite(reward.levelUp?'i-medal':'i-chest','reward-icon')+`<div><strong>${esc(reward.title)}</strong><span>${reward.items.map(esc).join(' · ')}</span></div>`;toastTimer=setTimeout(()=>el.classList.remove('show'),5200)}
...
function home(){
 ...
 return `<section class="campus-home ${state.preferences.onboarded?'returning-home':''}" aria-labelledby="home-title">
...
function pet(g){const p=activePet(g),lv=level(g),unlocked=gardenLevel(g);return `<div class="pet-layout">
 <section class="card pet-living"><div class="card-head"><h2 class="icon-heading tone-rose">${sprite('i-heart','heading-icon')}${esc(p.name)}的小屋</h2><span class="badge" data-tone="rose">亲密度 ${Math.floor(p.bond)} / 100</span></div><div class="petroom"><div class="window"></div><span class="pet-name">${p.sleeping?'晚安，做个好梦':'和你一起，慢慢长大'}</span><div class="room-shelf" aria-hidden="true">${sprite('i-book')}${sprite('i-mug')}</div>${g.equipped.includes('flower')?'<span class="flower"><img src="assets/garden/flora/berrybush.png" alt="窗边小花"></span>':''}${g.equipped.includes('lantern')?'<span class="lantern" aria-label="暖光灯笼"></span>':''}${cat(g)}${g.equipped.includes('scarf')?'<span class="scarf"></span>':''}</div>${p.say?`<p class="pet-say" role="status">${esc(p.say)}</p>`:''}<div class="care-actions">${btn('摸摸头','pat')}${btn('喂食 ×1','feed')}${btn('陪它玩','play')}${btn(p.sleeping?'叫醒':'休息','sleep')}</div><div class="meters">${[['hunger','饱食'],['energy','精力'],['mood','心情']].map(([k,n])=>`<div class="meter-line" data-meter="${k}"><span>${n}</span><div class="meter"><i style="width:${p[k]}%"></i></div><span>${Math.floor(p[k])}</span></div>`).join('')}</div><details id="pet-care-details" class="care-details"><summary>照料规则与农田点心</summary><p>摸摸间隔 10 秒，玩耍间隔 30 秒。每位伙伴每天前 3 次摸头获得成长，之后仍能开心互动。休息每小时恢复 30 点精力；离开几天也不会死亡。</p><div class="actions">${Object.entries(CROPS).map(([k,c])=>btn(`${c.name} ×${g.stock[k]}`,'feed',`data-crop="${k}" ${g.stock[k]?'':'disabled'}`)).join('')}</div><small>作物点心不消耗普通食物。</small></details></section>
 ...
 <section class="card keepsakes"><div class="card-head"><h2>小屋里的回忆</h2>${btn('翻开荔园拾光 →','gardenTab','data-tab="journal"','quiet')}</div>${[...journeyKeepsakes(g),...orderKeepsakes(g)].length?`<div class="keepsake-shelf">${[...journeyKeepsakes(g),...orderKeepsakes(g)].map(x=>`<span>${sprite(x.icon)}<strong>${esc(x.name)}</strong><small>${esc(x.storyTitle||'伙伴委托的纪念')}</small></span>`).join('')}</div>`:'<p class="muted">在「目标与图鉴」收下第一份小记忆，把它带回小屋。</p>'}</section>
...
function seedOptions(g){return Object.entries(CROPS).map(([k,c])=>`<option value="${k}" ${selectedCrop===k?'selected':''} ${gardenLevel(g)<c.level?'disabled':''}>${c.name} · ${g.seeds[k]} 颗${gardenLevel(g)<c.level?' · Lv.'+c.level+' 解锁':''}</option>`).join('')}
function farmDetails(g){const p=g.plots[selectedPlot];if(p==='locked'){const cost=60+(g.plots.filter(x=>x!=='locked').length-3)*30;return `<p>这里还长着青草，开垦后就能播种。</p><p class="farm-cost">${sprite('i-coin','item-icon')}需要 ${cost} 荔枝币 · 现有 ${Math.floor(g.coins)} 币</p>`}if(!p)return `<p>松松的泥土，等你种下下一份期待。</p><p class="farm-crop-info">${cropIcon(selectedCrop)}${CROPS[selectedCrop].name} · ${CROPS[selectedCrop].time/60000} 分钟 · 收获 ${CROPS[selectedCrop].yield} 个</p>`;return `<p class="farm-growth-state" data-ready="${p.ready}">${remaining(p.ready)}</p><p>收获 ${CROPS[p.crop].yield} 个${CROPS[p.crop].name} · ${p.watered?'已浇过水':plotGrowth(p)==='ready'?'可以放入收获篮':'浇水可缩短剩余时间的 25%'}</p>`}
...
function harvestBasket(g){return `<section class="card harvest-basket"><h2>我的收获篮</h2><div>${Object.entries(CROPS).map(([k,c])=>`<span>${cropIcon(k)}<strong>${c.name}</strong><small>× ${g.stock[k]}</small></span>`).join('')}</div>${btn('带去庭院集市 →','gardenTab','data-tab="market"','quiet')}</section>`}
function farm(g){return `<div class="farm-layout"><section class="farm-main"><div class="farm-toolbar"><div><h2>湖畔的六块小田</h2><p>点一块田，在农具箱里照料它。</p></div><span class="farm-toolbar-icon" aria-hidden="true">${sprite('i-water')}</span></div><div class="farm-landscape"><div class="farm-location">文山湖畔 · 我的小菜园</div><div class="plots" role="group" aria-label="选择要照料的农田">${g.plots.map((p,i)=>`<button type="button" class="plot" data-action="selectPlot" data-index="${i}" data-farm-plot="${i}" data-growth="${plotGrowth(p)}" aria-pressed="${selectedPlot===i}" aria-controls="farm-controls" aria-label="${plotLabel(p,i)}">${plotContents(p,i)}</button>`).join('')}</div><p class="farm-kind">成熟后不会枯萎，忙完再来收也没关系。</p></div><details class="farm-help"><summary>种植小贴士</summary><p>每块地浇水一次，缩短剩余时间的 25%。小萝卜 1 分钟成熟；收成可以喂给伙伴，也可以在集市出售。荔枝币仅用于本机庭院，没有充值、提现或交易功能。</p></details></section>
 <section class="card farm-controls" id="farm-controls" aria-labelledby="farm-selected-title"><p class="eyebrow">${sprite('i-water','item-icon')}我的农具箱</p><h2 id="farm-selected-title" aria-live="polite">第 ${selectedPlot+1} 块田</h2><div id="farm-plot-details">${farmDetails(g)}</div><div class="seed-picker"><label for="seed-choice">下一次播种</label><select id="seed-choice">${seedOptions(g)}</select><small>每次播种消耗 1 颗种子</small></div><div class="actions" id="farm-plot-actions" aria-live="polite">${farmAction(g)}</div></section>
 <aside class="farm-sidebar">${dailyBoard(g)}${harvestBasket(g)}</aside></div>`}
...
function paintDay(previousDay){
 ...
 if(page==='garden'&&gardenTab==='market'){const el=document.querySelector('.garden-orders');if(el)el.outerHTML=ordersView(g,{cropIcon,crops:CROPS})}
...
function petActionMessage(action){const pet=activePet(state.game);return {pat:'摸摸头，它很开心',feed:'吃饱啦，谢谢你',play:'玩得很开心！',chat:pet.say,sleep:pet.sleeping?'晚安，'+pet.name:pet.name+'醒来啦',switchPet:pet.name+'来陪你啦'}[action]}
