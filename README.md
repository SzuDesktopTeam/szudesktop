...
**以下改动尚未公开发布。上方下载仍是 beta0.9.2；beta0.9.3 本地安装包与便携包已构建。** 以下首页、农田、伙伴小屋和学习页截图来自同一候选程序与独立测试存档；关键浏览器路径和构建内容核对已通过，安装升级、系统通知与自启仍待实际验收。

代码已推送至 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)，候选包的 Windows 安装与升级检查可在该 PR 查看；公开下载尚未切换。
...
- **安静地留在桌面上。** 安装版源码支持专注完成通知、勿扰、宠物显隐/置顶记忆，以及主动开启的 Windows 自启；完全退出后不通知。最终安装包的实际行为仍待核验。
- **记住选择，也说清状态。** 公告来源和本科/研究生选择会保留；设置中可手动检查正式/测试版本、查看说明并复制无凭据的反馈信息。没有自动下载或安装更新。

25 个相关检查脚本及 Go 检查已通过；实际完成了专注、待办编辑归档、种植浇水收获、偏好恢复、版本查询与 PNG 预览保存。最终农田在 420px 窄窗可完整滚动且无横向溢出；候选文件、构建资源与许可材料已核对。完整美术统一、原生安装行为、学校业务及长期成长节奏仍需各自验收或试用。实施、校验值与证据统一见 [STATUS 第 55 节](docs/STATUS.md#55-产品体验实施与候选版准备2026-09-27源码未发布)。
