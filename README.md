...
**以下改动尚未公开发布。上方下载仍是 beta0.9.2；beta0.9.3 本地安装包与便携包已构建。** 以下首页、农田、伙伴小屋和学习页截图来自同一候选程序与独立测试存档；关键浏览器路径和构建内容核对已通过。CI 构建也已完成真实 Windows 安装升级与备份恢复，系统通知实际弹出和主动开启自启后的登录启动仍待验收。

代码 `09c2240` 已推送至草稿 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)，[全部 9 项 CI 工作通过](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36259475980)，包括 beta0.9.1 → beta0.9.3 安装升级、存档保留与备份恢复。PR 尚未合并，公开下载仍为 beta0.9.2。
...
[伙伴小屋](docs/screenshot-garden-preview.png) · [学习与周回顾](docs/screenshot-study-preview.png) · [实际导出的庭院明信片](docs/screenshot-share-preview.png)。新版兼容现有本机存档，并调整了成长规则；作物不会枯萎，每日目标没有连续签到要求。庭院背景由内置图片工具原创生成，随程序打包。
...
- **回来看一点新东西。** 七次非连续来访可翻开校园小故事、收下纪念物。四位伙伴都有普通、开心、低落和睡眠四种状态，四位伙伴共 56 条差异化照料台词；还可在本机预览并保存带校园背景、当前伙伴与真实庭院/专注数量的分享图，不包含学号、课表或成绩。
...
25 个相关检查脚本及 Go 检查已通过；实际完成了专注、待办编辑归档、种植浇水收获、偏好恢复、版本查询与 PNG 预览保存。最终农田在 420px 窄窗可完整滚动且无横向溢出；候选文件、构建资源与许可材料已核对。CI 构建已完成安装升级、卸载保留数据及桌宠交互检查，使用合成数据与单显示器。完整美术统一、通知与自启实际触发、学校业务、长期资源占用和成长节奏仍需各自验收或试用。实施、校验值与证据统一见 [STATUS 第 55 节](docs/STATUS.md#55-产品体验实施与候选版准备2026-09-27源码未发布)。
