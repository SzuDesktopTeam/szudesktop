...
**以下改动尚未公开发布，上方下载仍是 beta0.9.2。** 当前 beta0.9.3 候选把伙伴小桌、种植、委托、建设和收藏接进同一个庭院：玩一局带回需要的种子，收成送给伙伴，慢慢把湖边布置成自己的样子。源码仍在草稿 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)，本轮构建、实机画面与新 CI 结果统一更新到 [STATUS 第 58 节](docs/STATUS.md#58-庭院玩法闭环与下一步导览2026-09-27源码未发布)。此前提交 `a1685e0` 的 [9 项 CI 通过记录](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36264549047)只覆盖第 57 节的动画与小游戏基线，不代表本轮闭环已验收；下方旧候选截图也不替代新场景验收。
...
设置保存成功后会立即同步桌宠的动画开关，保存失败不会提前改变桌宠。新安装验收脚本先检查系统减少动态下的静态行为，再只对隔离测试渲染器模拟动画条件、检查真实帧推进并还原设置；**这项原生动态检查仍待本轮 CI 执行**，不会改用户的 Windows 设置。
...
![2048 合成菜园实机预览](docs/screenshot-arcade-preview.png)
