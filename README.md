...
**以下改动尚未公开发布，上方下载仍是 beta0.9.2。** 本轮将逐帧宠物、性格台词和小游戏接成可保存进度的庭院体验。32 组界面与桌面检查、真实浏览器试玩和本地构建已通过；功能提交 `a1685e0` 的 [9 项远端 CI 全部通过](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36264549047)，包括旧版安装升级、存档保留和桌宠交互。CI 的 Windows 系统开启了减少动态效果，因此实际帧推进在主窗口浏览器中验证，原生桌宠本次验证了静态模式及互动复位，未验证其动态播放。完整证据和候选包校验值统一记录在 [STATUS 第 57 节](docs/STATUS.md#57-逐帧伙伴与庭院小游戏2026-09-27源码未发布)。下方早期首页、农田和伙伴图片属于此前候选，不代表本轮动画的验收。
...
- **荔枝庭院 → 游戏角：合成菜园。** 完整 2048 规则，支持方向键、WASD、棋盘滑动与方向按钮，合并、新块有短动画；棋盘自动保存，可悔一步、重开，合到 2048 后继续。本局与最高分分别记录。首次合出 128 / 256 / 512 / 1024 / 2048 获得永久贴纸；当天实际合出 128 或更大数字，可领取一次 **8 币、2 亲密度**，旧棋盘留着大数字不会凭空获得新一天奖励。
- **荔枝庭院 → 庭院集市：伙伴的收获委托。** 每天固定三份需求，用现有收成交付；回报为材料原售价加 **3 / 3 / 4 币**答谢，一天额外奖励合计 10 币。交付 1 / 3 / 8 / 15 份会收到纪念物。材料不足会标出差额，完成后不能重复交付；没有连签或逾期惩罚。
