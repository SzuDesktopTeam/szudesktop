...
**以下改动尚未公开发布，上方下载仍是 beta0.9.2。** 当前 beta0.9.3 候选把伙伴小桌、种植、委托、建设和收藏接进同一个庭院：玩一局带回需要的种子，收成送给伙伴，慢慢把湖边布置成自己的样子。功能提交 `c355360` 已推至草稿 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)，[9 项 CI 全部通过](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36266684147)，包含真实跨版本安装升级和桌宠验收。本地 35 组检查通过，也已在真实页面走完备种、种植、收获、交付和建设。当前截图、候选安装包校验值及证据集中在 [STATUS 第 58 节](docs/STATUS.md#58-庭院玩法闭环与下一步导览2026-09-27源码未发布)。

### 伙伴开始有自己的动作和脾气

六种伙伴（含旧存档的阿青）各有 **12 组动作、每组 6 帧，共 432 帧**独立像素画：发呆、张望、散步、摸头、吃点心、玩耍、睡觉、伸懒腰、庆祝、陪伴专注、低落和打招呼。眼睛、嘴、四肢、尾巴与叶片分别变化；主窗口和桌宠使用同一个播放器，空闲节奏因角色而异，睡眠、专注和互动会切到对应动作。小屋的「动作册」可以逐组播放、查看每一帧。另保留 30 张静态形象，用于减少动态效果和导出明信片。关闭动画后，照料动作立即回到静态状态，后续睡眠或专注切换不会被旧动作卡住。

设置保存成功后会立即同步桌宠的动画开关，保存失败不会提前改变桌宠。安装验收已通过：先确认系统减少动态下保持静态，再只对隔离测试渲染器模拟允许动画，读到两个不同真实帧并截图；升级、重开和复用内核三条路径都通过，媒体条件与原偏好均已恢复。没有改动 Windows 设置。

**576 句台词覆盖 16 类情境**，每只伙伴每类 6 句，按各自进度轮换并存档。「聊两句」会看它是否睡着、饥饿、疲倦，以及早晚时间；摸头、喂食、收获、专注、待办与游戏结果也会回应。

| 伙伴 | 它怎么陪你 |
| :-- | :-- |
| 荔宝 | 热情、爱张罗，有一点冒失，总想搭把手 |
| 栗栗 | 认真看守纸箱，认为一切尽在掌握；保留抽象粗像素，也不知道自己丑萌 |
| 小白 | 文山湖边的白鹭，话不多，留意细小的变化 |
| Pingu | 好奇、孩子气，什么都想试试；高兴时偶尔 Noot |
| Skipper | 把小事当行动，把点心和休息也写进计划的企鹅队长 |
| 阿青 | 耐心、踏实，按自己的步子走；保留在旧存档「老朋友」里 |

![伙伴性格与逐帧动作册实机预览](docs/screenshot-pet-animation-preview.png)
...
开发者扩展宠物：稳定 ID 与上架状态登记在 `pet-catalog.mjs`，性格和情境语言在 `pet-dialogue.mjs`，逐帧画法在 `pet-animation-art.mjs`，时序和节奏在 `pet-animation.mjs`；主窗、桌宠自动复用 `pet-player.mjs`。新增角色须补齐静态形象、16 类台词和 12 组动画，运行对应检查。完整字段、导出 API 和打包依赖见 [STATUS 第 57.4 节](docs/STATUS.md#574-继续添加伙伴的统一契约)。
...
- **回来看一点新东西。** 七次非连续来访可翻开校园小故事、收下纪念物。伙伴静态四态继续保留，台词和动作已扩充为上文的 576 句与 432 帧；还可在本机预览并保存带校园背景、当前伙伴与真实庭院/专注数量的分享图，不包含学号、课表或成绩。
