...
**以下改动尚未公开发布，上方下载仍是 beta0.9.2。** 当前 beta0.9.3 源码新增三套可切换的三渲二首页风景，整合于 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)。接入与收尾验收记录集中在 [STATUS 第 61–62 节](docs/STATUS.md#62-首页收尾与合并验收2026-09-27)。第 60 节的本地候选安装包对应此前的 2048 素材版本。

### 换一扇窗，看见另一种荔园

点击首页顶部的**更换风景**，可以选择 **荔湖晴昼、雨后书屋、蓝调晚庭**，也能随时回到原来的**像素庭院**。选择会保存在本机，重新打开继续使用；伙伴、农田和待办共用原来的进度。选完后面板收起，把位置留给风景。

| 风景 | 窗外有什么 |
|---|---|
| 荔湖晴昼 | 湖岸弯道、南方棕榈、白红教学楼，以及长椅旁的自行车 |
| 雨后书屋 | 有进深的书店门面、书架和雨棚，盆栽旁留一处安静座位 |
| 蓝调晚庭 | 暮色里的层层露台、育苗架、暖窗与灯串 |

![荔湖晴昼三渲二首页的实际运行画面](docs/screenshot-home-lake-preview.png)

[查看雨后书屋](docs/screenshot-home-bookshop-preview.png) · [查看蓝调晚庭](docs/screenshot-home-terrace-preview.png)

这些校园主题小景由真实三维模型实时绘制。经 MIT 许可改编 [Sakura Crossing](https://github.com/Kenton-GMI/sakura-crossing) 的分层光照、彩色阴影、深度描线、天空和调色管线，使用随程序提供的 Three.js 0.180.0；场景几何为本项目新作，来源和改动见 [第三方说明](THIRD_PARTY_NOTICES.md)。

风景只在首页可见时绘制，离开页面会释放资源。设置里的「开启动画（伙伴与首页风景）」可切为静态画面，也遵守系统减少动态偏好；不支持立体渲染时保留像素校园和全部操作。

摸摸伙伴或保存待办时，背景继续使用同一块画布，保持动画连贯。伙伴平时只显示小名牌，回应后再自动收起气泡；暮色文字、远景树带和书屋铺地也完成了收尾调整。

底部提示条现在跟随当前伙伴：切换宠物、修改名字、切换页面和重新打开后，头像与名字保持一致。此修复已更新源码和本地预览，尚未纳入公开安装包；验收记录见 STATUS 第 60.4 节。

### 把收成摆上伙伴小桌

2048 的棋子现在来自同一座庭院：**2 种子袋 → 4 嫩芽 → 8 小萝卜 → 16 草莓 → 32 蓝莓 → 64 荔枝 → 128 萝卜篮 → 256 草莓篮 → 512 蓝莓篮 → 1024 荔枝篮 → 2048 荔宝丰收礼**。继续合到 4096 或以上会换上庆典装饰，完整数字始终保留。

农田与棋盘共用作物图案，移动中的棋子和停下后的棋子使用同一份画稿。桌边会提示「这局合到了什么、下一阶是什么」，完整素材图鉴默认折叠。原有合并规则、分数、存档、悔棋和每日种植补给均保持不变；棋盘图案不会直接变成农田库存，仍由领取补给把这一局接回种植。

![2048 庭院素材预览，使用隔离测试存档预置高阶棋盘](docs/screenshot-arcade-garden-preview.png)

这张截图用预置高阶棋盘检视 11 种图案，并非从零通关记录，也不代表真实游戏得分。

### 伙伴开始有自己的动作和脾气

六种伙伴（含旧存档的阿青）各有 **18 组动作、每组 6 帧，共 648 帧**独立像素画。原有日常动作之外，新增**浇水、抱回收成、递礼物、搭建、思考和拿手动作**；水壶、收成篮、小礼盒与小锤子有准备、动作和收势，眼睛、嘴、四肢、尾巴与叶片也分别变化。主窗口和桌宠使用同一个播放器，小屋「动作册」按日常、互动和庭院分组，可逐组播放、展开查看每一帧。另保留 30 张静态形象，用于减少动态效果和导出明信片。

**Pingu 重新画过了。** 黑脸小珠眼、短颈圆头、连到胸前的大白肚、鲜红突出嘴和橙色宽蹼足，共用一套画法覆盖静态与动画；高兴时嘴会真正张成喇叭。Skipper 继续保留独立的站姿、白脸和低眉，没有把两只企鹅画成换色版本。

设置保存成功后立即同步桌宠的动画开关，自定义时长开始专注也会立即同步；保存失败不会提前改变桌宠。睡眠和专注优先，拿手动作不会打断它们，减少动态时使用静态形象。本轮安装升级、重开和复用内核三条路径均通过：系统减少动态正常遵守，在隔离测试渲染器模拟允许动画后读取实际帧推进，最后恢复媒体条件与原偏好，没有改动 Windows 设置。

**1,656 句台词覆盖 23 类情境**，每只伙伴每类 12 句，按各自进度轮换并存档。新增播种、浇水、交付委托、建造、领取种子补给、开始专注和拿手动作的回应。栗栗继续一本正经地巡视纸箱，完全不知道自己丑萌；Pingu 会误会小东西，Skipper 连休息都当成后勤安排。台词按情境出现，不靠不停弹话增加存在感。

伙伴也来到农田旁边：播种、浇水、收获后可以直接看见它的动作与回应，不必回小屋才知道它说了什么。「看拿手动作」免费，不扣资源，也不增加成长或亲密度；本轮没有改变种植、委托或小游戏的经济规则。

| 伙伴 | 它怎么陪你 | 拿手动作 |
| :-- | :-- | :-- |
| 荔宝 | 热情、爱张罗，有一点冒失，总想搭把手 | 举起小牌为你加油 |
| 栗栗 | 认真看守纸箱，认为一切尽在掌握；保留抽象粗像素，也不知道自己丑萌 | 检查并坐进纸箱 |
| 小白 | 文山湖边的白鹭，话不多，留意细小的变化 | 低头理一理羽毛 |
| Pingu | 好奇、孩子气，什么都想试试；高兴时偶尔 Noot | 红嘴张成喇叭，Noot 一下 |
| Skipper | 把小事当行动，把点心和休息也写进计划的企鹅队长 | 举起望远镜巡查 |
| 阿青 | 耐心、踏实，按自己的步子走；保留在旧存档「老朋友」里 | 顶起一片小叶伞 |

![Pingu 重绘与拿手动作实机预览](docs/screenshot-pingu-refined-preview.png)

[查看六帧 Noot 动作册](docs/screenshot-pet-signatures-preview.png) · [查看农田陪伴与回应](docs/screenshot-pet-farm-preview.png)

### 从一颗种子，到湖边的一小块新布置

**专注与陪伴 → 备种和种植 → 收成 → 伙伴委托与庭院建设 → 真实场景和收藏。** 下列规则替代第 57 节初版小游戏奖励，庭院继续使用同一份种子、收成、荔枝币与伙伴成长。

- **伙伴小桌 · 一起备种。** 保留完整 2048、方向键 / WASD / 滑动、自动续盘、悔一步和最高纪录。当天实际合出 128 或以上，可领取一次**需求种子 ×1、8 币、成长 +3、亲密度 +2、心情 +10**，并计一次每日陪伴。种子优先补未完成委托和下一项建设的缺口，计算时包含篮子、已有种子及正在生长的作物；奖励不会给出尚未解锁的种子。领完可直接去播种，旧棋盘的大数字不能重复领取新一天奖励。
- **我的农田 · 收成有去处。** 选种时能看到哪些伙伴、哪项建设需要它；材料不足的委托直接带你去对应作物和空田。收成也能作为小屋点心喂给伙伴。集市「出售多余」会保留当天未完成委托与下一项建设所需的收成，再卖剩余数量。
- **庭院集市 · 送到真正提出委托的伙伴手里。** 每天三份固定需求，支付材料售价加 **3 / 3 / 4 币**答谢；提出委托的伙伴获得 **成长 +5、亲密度 +3**，不会错给当前选中的另一位伙伴。完成 1 / 3 / 8 / 15 份收下纪念物，无逾期惩罚。
- **回忆与建设 · 收成变成看得见的庭院。** 按下面的条件消耗材料与荔枝币建造，首次建成自动摆放；之后收起、摆出不再扣费。布置出现在首页、小屋和农田的像素场景中。
- **小屋收藏与「下一步」。** 2048 的五枚里程碑贴纸、委托纪念物和来访故事纪念物一起留在小屋。首页与庭院根据真实进度提示眼前可以做的一件事，直达成熟田块、可交付委托、建设、种子铺或专注；正在生长的作物计入准备进度。

| 庭院建设 | 开放条件 | 首次建造消耗 |
| :-- | :-- | :-- |
| 湖畔野餐角 | 累计收获 3 次 | 60 币 + 小萝卜 ×4 + 草莓 ×2 |
| 窗边育苗架 | 累计交付 3 份委托 | 120 币 + 草莓 ×4 + 蓝莓 ×3 |
| 湖畔灯径 | 累计交付 8 份委托 | 240 币 + 蓝莓 ×4 + 荔枝 ×3 |

原有「每日补给」仍是 20 币、食物 ×1、萝卜种子 ×2，与伙伴小桌的每日备种礼分别领取。喜欢专注、种植或照料也能继续建设庭院，2048 不设强制通关门槛。

![庭院建设与下一步目标实机预览](docs/screenshot-garden-goal-preview.png)

[查看农田与已建野餐角](docs/screenshot-garden-loop-preview.png)
...
新增 **Pingu** 和《马达加斯加》的企鹅队长 **Skipper**：一位摇摇晃晃地 Noot Noot，一位认真执行“陪你休息”的任务。两只企鹅都有普通、开心、低落和睡眠四态。保留文山湖白鹭小白；栗栗恢复原来粗像素、很抽象的样子，一本正经地占着纸箱，并不知道自己丑萌。新存档默认五位伙伴，旧存档里的阿青仍在「老朋友」中，名字、成长和当前选择都保留。

![新增企鹅与抽象栗栗的源码预览，尚未公开发布](docs/screenshot-pets-preview.png)
...
- **回来看一点新东西。** 七次非连续来访可翻开校园小故事、收下纪念物。伙伴静态四态继续保留，台词和动作已扩充为上文的 1,656 句与 648 帧；还可在本机预览并保存带校园背景、当前伙伴与真实庭院/专注数量的分享图，不包含学号、课表或成绩。
