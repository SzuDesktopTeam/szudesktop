...
**以下改动尚未公开发布，上方下载仍是 beta0.9.2。** 当前 beta0.9.3 候选继续打磨同一个庭院：重绘 Pingu，给每位伙伴补上庭院动作、拿手动作和更多贴近眼前事情的回应。沿用草稿 [PR #19](https://github.com/SzuDesktopTeam/szudesktop/pull/19)。**本轮 36 组本地检查通过，真实页面与 420px 窄窗已核对，Windows 候选包已重建；远端 CI 待本次推送后确认**，进度集中在 [STATUS 第 59 节](docs/STATUS.md#59-pingu-形象与伙伴生活反应2026-09-27源码未发布)。
...
设置保存成功后会立即同步桌宠的动画开关，保存失败不会提前改变桌宠。睡眠和专注优先，随手查看拿手动作不会打断它们；减少动态效果时使用静态形象。上一轮 `c355360` 安装验收已确认系统减少动态下保持静态，并只在隔离测试渲染器模拟允许动画，读到两个真实帧；升级、重开和复用内核三条路径均恢复了媒体条件与原偏好，没有改动 Windows 设置。本轮新增动作的完整安装验收待补。
